<%@ Page Title="" Language="C#" MasterPageFile="~/Accounting/MasterPageAccounting.master" AutoEventWireup="true" CodeFile="BillReceivedPage.aspx.cs" Inherits="Accounting_BillReceivedPage" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="contentfolder">
		...
		<table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" class="tblBg">
		    ...
		    <tr> 
			  <td width="20%" class="tblTextCommon">No. Bil:</td>
			  <td width="80%" class="tblText2">
                  <input id="txtFindExpensesNo" name="txtFindExpensesNo" type="text" size="20" maxlength="20" value="<%=sCurrExpensesNo %>" class="input">
                  <div id="txtFindExpensesNo-container" style="position: relative; float: left; width: 100%; margin: 0px; background-color: aqua;"></div>
              </td>
			</tr>
            <tr>
        ...
        <table width="98%" border="0" cellspacing="1" cellpadding="5" align="center">
            <tr>
                <td align="center" valign="top" ><a href="#" class="activeTab tab">Senarai Transaksi Bil Diterima / List of Bill Received Transaction</a></td>
            </tr>
        </table>
        ...
        <table id="mytable" width="98%" border="0" cellspacing="1" cellpadding="2" class="tblBg fixed_header" align="center">
                <tbody>
                    <tr class="tblTitle3Mod">
                        ...
                        <td width="6%" valign="middle" align="left" class="tblTitle3Mod">No. Bil</td>
                        ...
                        <td width="20%" valign="middle" align="left" class="tblTitle3Mod">Pembekal</td>
                    ...
                    <%
                        if (lsBillReceived.Count > 0)
                        {
                            for (int i = 0; i < lsBillReceived.Count; i++)
                            {
                                MainModel modMain = (MainModel)lsBillReceived[i];
                    ...
                    <tr data-id="<%=modMain.GetSetinvoiceno %>_<%=modMain.GetSetlineno %>" class="tblText1">
                        ...
                        <td valign="top" align="left"><a href="#" onclick="openeditexpenses('<%=modMain.GetSetcomp%>', '<%=modMain.GetSetexpensesno%>');"><%=modMain.GetSetexpensesno %></a></td>
                        <td valign="top" align="left"><%=modMain.GetSetexpensescat %></td>
                        ...
                        <td valign="top" align="left"><%=modMain.GetSettotalexpenses %></td>
                ...
                <tr>
                    <td>
                        <table width="100%" cellpadding="2" cellspacing="1" class="tblTextMod">
                            <tr>
                                <td width="50%" height="15" align="left"><%=lsBillReceived.Count %> record(s)</td>
	...
	<script type="text/javascript">
        ...
        function actionclick(action) {
            var proceed = true;
            if (action == "RESET") {
                document.getElementById("txtFindExpensesNo").value = "";
            }
            if (proceed) {
        ...
        $(document).ready(function () {
            ...
            var getExpensesList_parameters = ["currcomp", "<%=sCurrComp%>", "fyr", "<%=sCurrFyr %>"];
            PageMethod("getExpensesList", getExpensesList_parameters, getExpensesList_succeedAjaxFn, getExpensesList_failedAjaxFn, false);
        ...
        var getExpensesList_succeedAjaxFn = function (data, textStatus, jqXHR) {
            console.log("getExpensesList_succeedAjaxFn: " + textStatus);
            var getExpensesList_result = JSON.parse(data.d);
            if (getExpensesList_result.result == "Y") {
                $.each(getExpensesList_result.expenseslist, function (i, result) {
                    var objData = {};
                    objData.value = result.GetSetexpensesno + '-' + result.GetSetbpdesc;
                    objData.data = result.GetSetexpensesno;
                    expensesnoArray.push(objData);
                ...
                console.log("getExpensesList_result.result: " + getExpensesList_result.result);
            }
        }

        var getExpensesList_failedAjaxFn = function (jqXHR, textStatus, errorThrown) {
            console.log("getExpensesList_failedAjaxFn: " + textStatus);
        }

        $('#txtFindExpensesNo').autocomplete({
            lookup: expensesnoArray,
            appendTo: '#txtFindExpensesNo-container',
            minLength: 0,
            minChars: 0,
            width: maxlengthdataautocomplete * 12,
            onSelect: function (suggestion) {
                $('#txtFindExpensesNo').val(suggestion.data);
            }
        });
