<%@ Page Title="" Language="C#" MasterPageFile="~/Accounting/MasterPageAccounting.master" AutoEventWireup="true" CodeFile="BalanceSheetPage.aspx.cs" Inherits="Accounting_BalanceSheetPage" %>
    ...
    <div class="contentfolder">
        <form id="form1" runat="server">
            ...
            <table width="98%" border="0" cellspacing="1" cellpadding="5" align="center" style="border-spacing: 1px; border-collapse: inherit;">
                <tr>
                    <td align="center" valign="top"><a href="#" class="activeTab tab">Laporan Penyata Kedudukan Kewangan / Balance Sheet Report</a></td>
                </tr>
            </table>
            ...
            <table id="mytable" width="98%" border="0" cellspacing="1" cellpadding="2" class="tblBg fixed_header" align="center">
                <tbody>
                    ...
                    <%
                        if (lsBalanceSheet.Count > 0)
                        {
                            for (int i = 0; i < lsBalanceSheet.Count; i++)
                            {
                                AccountingModel modAcc = (AccountingModel)lsBalanceSheet[i];
                    ...
                    <tr data-id="<%=modAcc.GetSetid %>" class="tblText1">
                        ...
                        <td valign="top" align="center">
                            <button type="button" data-action="open_trans" class="button_warning" onclick="openBalanceSheetDetails('OPEN','<%=modAcc.GetSetfyr %>','<%=modAcc.GetSetitemtype %>', '<%=modAcc.GetSetitemtypedesc %>','<%=modAcc.GetSetdatefrom %>', '<%=modAcc.GetSetdateto %>', '<%=modAcc.GetSetcurrency %>', '<%=modAcc.GetSetstatus %>');">Senarai Penyata Kewangan</button>
                        </td>
                    </tr>
                    <%
                            }
                ...
                <tr>
                    <td>
                        <table width="100%" cellpadding="2" cellspacing="1" class="tblTextMod">
                            <tr>
                                <td width="50%" height="15" align="left"><%=lsBalanceSheet.Count %> record(s)</td>
