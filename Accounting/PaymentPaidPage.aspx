<%@ Page Title="" Language="C#" MasterPageFile="~/Accounting/MasterPageAccounting.master" AutoEventWireup="true" CodeFile="PaymentPaidPage.aspx.cs" Inherits="Accounting_PaymentPaidPage" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
		...
		<table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" class="tblBg">
		    ...
		    <tr> 
			  ...
			  <td width="80%" class="tblText2">
                  <input id="txtFindPaidNo" name="txtFindPaidNo" type="text" size="20" maxlength="20" value="<%=sCurrPayPaidNo %>" class="input">
                  <div id="txtFindPaidNo-container" style="position: relative; float: left; width: 100%; margin: 0px; background-color: aqua;"></div>
              </td>
			</tr>
            <tr>
        ...
        <table id="mytable" width="98%" border="0" cellspacing="1" cellpadding="2" class="tblBg fixed_header" align="center">
                <tbody>
                    ...
                    <tr data-id="<%=modMain.GetSetinvoiceno %>_<%=modMain.GetSetlineno %>" class="tblText1">
                        ...
                        <td valign="top" align="left"><a href="#" onclick="openeditpaymentpaid('<%=modMain.GetSetcomp%>', '<%=modMain.GetSetpaymentno%>');"><%=modMain.GetSetpaymentno %></a></td>
	...
	<script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var getPayPaidList_parameters = ["currcomp", "<%=sCurrComp%>", "fyr", "<%=sCurrFyr %>"];
            PageMethod("getPayPaidList", getPayPaidList_parameters, getPayPaidList_succeedAjaxFn, getPayPaidList_failedAjaxFn, false);
        ...
        var getPayPaidList_succeedAjaxFn = function (data, textStatus, jqXHR) {
            console.log("getPayPaidList_succeedAjaxFn: " + textStatus);
            var getPayPaidList_result = JSON.parse(data.d);
            if (getPayPaidList_result.result == "Y") {
                $.each(getPayPaidList_result.payrcptlist, function (i, result) {
                    var objData = {};
                    objData.value = result.GetSetpaypaidno + '-' + result.GetSetbpdesc;
                    objData.data = result.GetSetpaypaidno;
                    paypaidnoArray.push(objData);
                ...
                console.log("getPayPaidList_result.result: " + getPayPaidList_result.result);
            }
        }
        ...
        $('#txtFindPaidNo').autocomplete({
            lookup: paypaidnoArray,
            appendTo: '#txtFindPaidNo-container',
            minLength: 0,
            minChars: 0,
            width: maxlengthdataautocomplete * 12,
            onSelect: function (suggestion) {
                $('#txtFindPaidNo').val(suggestion.data);
            }
        });
