...
public partial class Accounting_PaymentPaidPage : System.Web.UI.Page
{
    ...
    public ArrayList lsPayPaidNo = new ArrayList();
    ...
    private void processValues()
    {
        ...
        lsPaymentReceipt = oMainCon.getPaymentPaidCashFlowList(sCurrComp, sCurrFyr, datetime1.ToString(), datetime2.ToString(), "CONFIRMED");
    ...
    private void getValues()
    {
        ...
        if (Request.Params.Get("txtFindPaidNo") != null)
        {
            sCurrPayPaidNo = oAccCon.replaceNull(Request.Params.Get("txtFindPaidNo"));
        }
    ...
    [WebMethod(EnableSession = true)]
    public static String getPayPaidList(string currcomp, string fyr)
    {
        ...
        if (currcomp.Length > 0)
        {
            ArrayList lsPayPaid = oMainCon.getPaymentPaidHeaderList(currcomp, "", "", "01-01-" + fyr, "31-12-" + fyr, "CONFIRMED");
            for (int i = 0; i < lsPayPaid.Count; i++)
            {
                MainModel oMod = (MainModel)lsPayPaid[i];

                Object objData = new
                {
                    GetSetpaypaidno = oMod.GetSetpaypaidno,
                    GetSetbpdesc = oMod.GetSetbpdesc + " [" + oMod.GetSetpayrcptamount + "]"
                };
                lsPayPaidNo.Add(objData);
            }
            sStatus = "Y";
        }

        object retData = new { result = sStatus, payrcptlist = lsPayPaidNo };
