...
public partial class Accounting_BillReceivedPage : System.Web.UI.Page
{
    ...
    public ArrayList lsBillReceived = new ArrayList();
    public ArrayList lsExpensesNo = new ArrayList();
    ...
    private void processValues()
    {
        ...
        lsBillReceived = oMainCon.getExpensesHeaderDetailsList(sCurrComp, sCurrFyr, sCurrExpensesNo, "", datetime1.ToString(), datetime2.ToString(), "", "CONFIRMED", "");
    ...
    private void getValues()
    {
        ...
        if (Request.Params.Get("txtFindInvoiceNo") != null)
        {
            sCurrExpensesNo = oAccCon.replaceNull(Request.Params.Get("txtFindExpensesNo"));
        }
    ...
    [WebMethod(EnableSession = true)]
    public static String getExpensesList(string currcomp, string fyr)
    {
        MainController oMainCon = new MainController();
        String sUserId = "";
        String sStatus = "N";

        ArrayList lsExpensesNo = new ArrayList();
        ...
        if (currcomp.Length > 0)
        {
            ArrayList lsInvoice = oMainCon.getExpensesHeaderList(currcomp, "", "", "01-01-" + fyr, "31-12-" + fyr, "CONFIRMED");
            for (int i = 0; i < lsInvoice.Count; i++)
            {
                MainModel oMod = (MainModel)lsInvoice[i];

                Object objData = new
                {
                    GetSetexpensesno = oMod.GetSetexpensesno,
                    GetSetbpdesc = oMod.GetSetbpdesc + " [" + oMod.GetSettotalamount + "]"
                };
                lsExpensesNo.Add(objData);
            }
            sStatus = "Y";
        }

        object retData = new { result = sStatus, expenseslist = lsExpensesNo };
