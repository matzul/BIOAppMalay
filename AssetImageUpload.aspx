<%@ Page Language="C#" AutoEventWireup="true" CodeFile="AssetImageUpload.aspx.cs" Inherits="AssetImageUpload" %>
...
<body>
    ...
    <div class="section section--head">
        ...
        <div class="row row--heading">
            <h1><%=sAssetNo %></h1>
            <p><%=oModAsset.GetSetassetdesc %></p>
        </div>

        <div class="row">
            <div id="demo-test-gallery" class="demo-gallery">
                <%
                    if (lsAssetImage.Count > 0)
                    {
                        for (int i = 0; i < lsAssetImage.Count; i++)
                        {
                            MainModel assetImage = (MainModel)lsAssetImage[i];
                %>
                
                <a href='./Attachment/<%=assetImage.GetSetfilename %>' data-author="<%=assetImage.GetSetfilename %>" data-size='<%=assetImage.GetSetimgwidth %>x<%=assetImage.GetSetimgheight %>' data-med='./Attachment/<%=assetImage.GetSetfilename %>' data-med-size='<%=assetImage.GetSetimgwidth %>x<%=assetImage.GetSetimgheight %>' <%=i.Equals(0)?"class='demo-gallery__img--main'":"" %>>
                    <img src='./Attachment/<%=assetImage.GetSetfilename %>' alt='' />
                    <figure><%=sAssetNo %></figure>
                </a>
                <%
                        }
                    }
    ...
    <script type="text/javascript">
        ...
        function uploadFile(file, i) {
            ...
            formData.append('inputitem[2]', '<%=sAssetNo%>');
            formData.append("inputitem[3]", file);

            if ("<%=sUserId%>" != "") {
                $.ajax({
                    type: 'POST',
                    url: "WebService.asmx/uploadFileAsset",
        ...
        function storeFile() {
            var pagePath = window.location.hostname;
            //var paramList = "{'comp':'<%=sCurrComp%>', 'userid':'<%=sUserId%>', 'itemno':'<%=sAssetNo%>','filename':" + document.getElementById("filename").value + "}";
            var formData = new FormData();
            formData.append('inputitem[0]', '<%=sCurrComp%>');
            formData.append('inputitem[1]', '<%=sUserId%>');
            formData.append('inputitem[2]', '<%=sAssetNo%>');
            ...
            if ("<%=sUserId%>" != "") {
                $.ajax({
                    type: "POST",
                    url: "WebService.asmx/storeFileAsset",
                    ...
                    success: function (data, status, xmlData) {
                        ...
                        if (result.status == 'Y') {
                            ...
                            window.location.href = "AssetImageUpload.aspx?action=OPEN&assetno=<%=sAssetNo %>";
                        }
                        else {
        ...
        function deleteFile(filename) {
            ...
            if ("<%=sUserId%>" != "") {
                $.ajax({
                    type: "POST",
                    url: "WebService.asmx/deleteFileAsset",
