...
public partial class CompStaffDetails : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static String updateStaffDetails(string currcomp, string staffno, string salute, string nickname, string nationality, string dob, string birthplace, string gender, string race, string religion,
        string marital, string dtmarried, string bankname, string accountno, string epfno, string socsono, string taxno, int retireage, string userid, string password, 
        string paddress1, string paddress2, string paddress3, string paddress4, string ppostcode, string pcity, string pstate, string pcountry, string ptelephone, string facebook,
        string instagram, string whatsapp, string remarks)
    {
        HRController oHRCon = new HRController();
        String sUserId = "";
        String sStatus = "N";
        ...
        {
            HRModel modItem = oHRCon.getStaffDetails(currcomp, staffno);

            if (modItem.GetSetid > 0)
            {
                modItem.GetSetsalute = salute;
                modItem.GetSetnickname = nickname;
                modItem.GetSetnationality = nationality;
                modItem.GetSetdob = dob;
                modItem.GetSetbirthplace = birthplace;
                modItem.GetSetgender = gender;
                modItem.GetSetrace = race;
                modItem.GetSetreligion = religion;
                modItem.GetSetmarital = marital;
                modItem.GetSetdtmarried = dtmarried;
                modItem.GetSetbankname = bankname;
                modItem.GetSetaccountno = accountno;
                modItem.GetSetepfno = epfno;
                modItem.GetSetsocsono = socsono;
                modItem.GetSettaxno = taxno;
                modItem.GetSetretireage = retireage;
                modItem.GetSetuserid = userid;
                modItem.GetSetpassword = password;
                modItem.GetSetpaddress1 = paddress1;
                modItem.GetSetpaddress2 = paddress2;
                modItem.GetSetpaddress3 = paddress3;
                modItem.GetSetpaddress4 = paddress4;
                modItem.GetSetppostcode = ppostcode;
                modItem.GetSetpcity = pcity;
                modItem.GetSetpstate = pstate;
                modItem.GetSetpcountry = pcountry;
                modItem.GetSetptelephone = ptelephone;
                modItem.GetSetfacebook = facebook;
                modItem.GetSetinstagram = instagram;
                modItem.GetSetwhatsapp = whatsapp;
                modItem.GetSetremarks = remarks;
                modItem.GetSetmodifiedby = sUserId;
                String result = oHRCon.updateStaffInfo(modItem);
                if (result.Equals("Y"))
                {
                    sStatus = "Y";
            ...
            {
                sStatus = "N";
                sMessage = "Kemaskini tidak berjaya! Record not found for Comp: " + currcomp + " & StaffNo: " + staffno;
            }
        }
