...
public partial class HumanResource_AttendanceWorkingGeneralTableStaff : System.Web.UI.Page
{
    ...
    public List<HRModel> lsExceptionDayList = new List<HRModel>();
    public List<HRModel> lsLeaveDayList = new List<HRModel>();
    ...
    private void initialValues()
    {
        ...
        {
            sId = oHRCon.replaceNull(Request.QueryString["id"]);
        }
        if (Request.QueryString["typ"] != null)
        {
            sType = oHRCon.replaceNull(Request.QueryString["typ"]);
        }
    ...
    private void processValues()
    {
        ...
        if (sType.Equals("FROM_EXCUSE"))
        {
            modWorkingGroupStaff = oHRCon.getStaffExcuseDetails(sCurrComp, sCurrFyr, "", "", "", "", "", int.Parse(sId.Length > 0 ? sId : "0"), "");
        }
        else if (sType.Equals("FROM_LEAVE"))
        {
            modWorkingGroupStaff = oHRCon.getStaffLeaveDetails(sCurrComp, sCurrFyr, "", "", "", "", "", int.Parse(sId.Length > 0 ? sId : "0"), "");
        }
        else
        {
            modWorkingGroupStaff = new HRModel();
        }
        if (modWorkingGroupStaff.GetSetstaffno.Length > 0)
        {
            //lsExceptionDayString = oHRCon.getStaffExceptionList(modWorkingGroupStaff.GetSetcomp, modWorkingGroupStaff.GetSetfyr, modWorkingGroupStaff.GetSetstaffno, "", "", "", 0);
            lsExceptionDayList = oHRCon.getStaffExceptionStringList(modWorkingGroupStaff.GetSetcomp, modWorkingGroupStaff.GetSetfyr, modWorkingGroupStaff.GetSetstaffno, "", "", "", 0);
            lsLeaveDayList = oHRCon.getStaffLeaveStringList(modWorkingGroupStaff.GetSetcomp, modWorkingGroupStaff.GetSetfyr, modWorkingGroupStaff.GetSetstaffno, "", "", "", 0);
