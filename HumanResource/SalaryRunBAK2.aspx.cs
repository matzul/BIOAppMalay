...
public partial class HumanResource_SalaryRun : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static String updateSGItemDetail(string currcomp, string currfyr, Int64 ss_id, HRModel[] inputarray)
    {
        HRController oHRCon = new HRController();
        String sUserId = "";
        String sStatus = "N";
        ...
        if (currcomp.Length > 0 && ss_id > 0)
        {
            ...
            if (modItem.GetSetid > 0)
            {
                ...
                for (int i = 0; i < lsItem.Count; i++)
                {
                    ...
                    ArrayList lsSalaryItem = oHRCon.getRunStaffSalaryItemList(currcomp, currfyr, oHRMod.GetSetstaffno, 0, 0, oHRMod.GetSetss_id, oHRMod.GetSetid, "", "", "");
                    /*
                    for (int j = 0; j < lsSalaryItem.Count; j++)
                    {
                        HRModel oHRModItem = (HRModel)lsSalaryItem[j];
                    }
                    */
                    if(lsSalaryItem.Count > 0)
                    {
                        //generatePDFFile(oHRMod, lsSalaryItem);
                    }
