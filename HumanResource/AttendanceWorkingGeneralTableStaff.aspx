<%@ Page Title="" Language="C#" MasterPageFile="~/HumanResource/MasterPageAttendanceChild.master" AutoEventWireup="true" CodeFile="AttendanceWorkingGeneralTableStaff.aspx.cs" Inherits="HumanResource_AttendanceWorkingGeneralTableStaff" %>
            ...
            <table id="mytable" width="98%" border="0" cellspacing="1" cellpadding="2" class="tblBg fixed_header" align="center">
                <tbody>
                    <tr class="tblTitle3Mod">
                        ...
                        <td width="15%" valign="middle" align="left" class="tblTitle3Mod">Tarikh/ Hari</td>
                        <td width="14%" valign="middle" align="left" class="tblTitle3Mod">Masa Kerja</td>
                        ...
                        <td width="15%" valign="middle" align="left" class="tblTitle3Mod">Cuti/ Pengecualian</td>
                    </tr>
                    <%
                        if (lsWorkingGroupStaffDayAll.Count > 0)
                        {
                            for (int i = 0; i < lsWorkingGroupStaffDayAll.Count; i++)
                            {
                                ...
                                //HRModel modExceptionDay = oHRCon.getStaffExceptionDetails(modWorkStaffDay.GetSetcomp, modWorkStaffDay.GetSetfyr, modWorkStaffDay.GetSetstaffno, modWorkStaffDay.GetSetday_date, "", "", 0, 0);
                                //var boolexceptday = oHRCon.isBoolContains(lsExceptionDayString, modWorkStaffDay.GetSetday_date);
                                var boolexceptday = lsExceptionDayList.Where(p => p.GetSetday_date.Contains(modWorkStaffDay.GetSetday_date));
                                Int64 exceptid = 0;
                                var exceptcat = "";
                                var excepttype = "";
                                foreach (var result in boolexceptday)
                                {
                                    //Console.WriteLine("Product Name: {0}", result);
                                    exceptid = result.GetSetid;
                                    exceptcat = result.GetSetcat;
                                    excepttype = result.GetSettype;
                                }

                                var boolleaveday = lsLeaveDayList.Where(p => p.GetSetday_date.Contains(modWorkStaffDay.GetSetday_date));
                                Int64 leaveid = 0;
                                var leavecat = "";
                                var leavetype = "";
                                foreach (var result in boolexceptday)
                                {
                                    //Console.WriteLine("Product Name: {0}", result);
                                    leaveid = result.GetSetid;
                                    leavecat = result.GetSetcat;
                                    leavetype = result.GetSettype;
                                }
                    ...
                    <tr data-depth="1" class="level1 <%=boolrestday.Equals(true)?"tblTitle2":boolphday.Equals(true)?"hilite1":"tblText1" %>" data-id="<%=modWorkStaffDay.GetSetwd_id %>">
                        ...
                        <td valign="top" align="left"><%=(exceptid>0?exceptcat+" - "+excepttype:"")%><br /><%=(leaveid>0?leavecat+" - "+leavetype:"")%></td>
                    </tr>
                    <%
                                }
                                else
                    ...
                    <tr data-depth="1" class="level1 <%=boolrestday.Equals(true)?"tblTitle2":boolphday.Equals(true)?"hilite1":"tblText1" %>" data-id="<%=modWorkStaffDay.GetSetwd_id %>">
                        ...
                        <td valign="top" align="left"><input type="time" id="timeout_<%=modWorkStaffDay.GetSetwd_id %>" value="<%=modWorkStaffDay.GetSettimeout %>" class="timeout" step="1"></td>
                        <td valign="top" align="left"><%=boolrestday.Equals(true)?"REST-DAY":boolphday.Equals(true)?"OFF-DAY":"" %></td>
                        <td valign="top" align="left"><%=(exceptid>0?exceptcat+" - "+excepttype:"")%></td>
                    </tr>
                    <%                            
                                }                                               
                    %>
