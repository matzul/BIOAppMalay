...
var Graph = Graph || (function($) {
	...
	Private.basics = function(height, width, graphHeight, graphWidth) {
		...
		return {
			...
			yDist: 30,
		    scale: 10,
			//scale: Private.getCei,//untuk y axis count
...
(function($) {
	$.fn.graphify = $.fn.graphify || function(options) {
	    ...
	    //var specials = ['area', 'combo', 'donut']; //subsets of major graphs //, 'table'
		//if graph has multiple datasets we can not make a pie graph or combo graph://, 'combo'
		if ($.isArray(self.points[0])) {
			types = ['linear', 'area', 'bar'];
		} else {
